    ...
    async with aiosqlite.connect(DB_PATH) as db:
        if season is None:
            cur = await db.execute(
                "SELECT telegram_message_id, title FROM entries WHERE imdb_id=? AND season IS NULL",
                (imdb_id,)
            )
        else:
            cur = await db.execute(
                "SELECT telegram_message_id, title FROM entries WHERE imdb_id=? AND season=? AND episode=?",
...
async def search_fimoo(imdb_id, season=None, episode=None):
    query = f"{imdb_id}:{season}:{episode}" if season is not None else imdb_id
    try:
...
async def add_movie(event):
    if event.sender_id != ADMIN_USER_ID or not event.is_reply:
        return
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            "INSERT OR REPLACE INTO entries (content_type, imdb_id, telegram_message_id, title) VALUES ('movie', ?, ?, ?)",
...
async def add_series(event):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            "INSERT OR REPLACE INTO entries (content_type, imdb_id, season, episode, telegram_message_id, title) VALUES ('series', ?, ?, ?, ?, ?)",
            (imdb_id, int(s), int(e), replied.id, title)
        )
        await db.commit()
